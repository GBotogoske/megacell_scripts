...
void fit_hist_charge_3(string filename,int n_channel=0)
{
	...
	for(;peak_actual<=n_peaks;peak_actual++)
	{
		...
		gaus_fit[peak_actual-1]->SetParLimits(0,1.5e3,5e3);	
		gaus_fit[peak_actual-1]->SetParLimits(1,-10,300);
		gaus_fit[peak_actual-1]->SetParLimits(2,300,700);
		...
		if(peak_actual>=2)
		{
			...
			gaus_fit[peak_actual-1]->SetParLimits(3,1.4e3,1.7e3);
		...
		//cout<<lambda<<endl;
			...
			for(int i=1;i<peak_actual;i++)
			{
				gaus_fit[peak_actual-1]->SetParLimits(3+2*i-1,0.01,2.5e3);
				gaus_fit[peak_actual-1]->SetParLimits(3+2*i,80,1.8e3);
			}
		}
		...
		if(peak_actual==draw)
		{
			...
			for(int i=0;i<peak_actual;i++)
			{
				gaus_peaks[i]= new TF1(Form("gaus_peak%i",i),"gaus",min_fit,max_fit);
				if(i==0)
				{
	...
	//cria arquivo
	string new_name = to_string(n_channel)+"/fit.txt";
	...
	string new_name_2 = to_string(n_channel)+"/data.txt";
	...
	for(int i=0;i<n_peaks;i++)
	{
		if(i==0)
		{
			legend->AddEntry(gaus_peaks[i],Form("-- 0 spe: A0: %.2f -- mean0: %.2f -- std: %.2f",param[0],param[1],param[2]),"f");
		}
		else if(i==1)
		{
			legend->AddEntry(gaus_peaks[i],Form("-- 1 spe: A1: %.2f -- mean_spe: %.2f -- std_spe: %.2f",param[4],param[3],param[5]),"f");
		}
		else
		{
			legend->AddEntry(gaus_peaks[i],Form("-- %i spe: A%i: %.2f -- mean_spe*%i: %.2f -- std_%i: %.2f",i,i,param[3+2*i-1],i,param[3]*i,i,param[3+2*i]),"f");
		}
	...
	c->SaveAs(Form("%d/spe.pdf",n_channel));
